...
meta = [
  ...
  "argument_groups" : [
    {
      ...
      "arguments" : [
        ...
        },
        {
          ...
          "description" : "LightGBM learning rate. The original solution used 0.01 with up to 10000 rounds; 0.1 with a few\nhundred rounds reaches the same LightGBM quality in a fraction of the time.\n",
          "default" : [
            0.1
          ],
          "required" : false,
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_predict_modality/task_predict_modality/src/methods/senkin_tmp/senkin_tmp_train/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
...
par = {
  ...
  'n_folds': $( if [ ! -z ${VIASH_PAR_N_FOLDS+x} ]; then echo "int(r'${VIASH_PAR_N_FOLDS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_n_folds': $( if [ ! -z ${VIASH_PAR_LGBM_N_FOLDS+x} ]; then echo "int(r'${VIASH_PAR_LGBM_N_FOLDS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_boost_rounds': $( if [ ! -z ${VIASH_PAR_LGBM_BOOST_ROUNDS+x} ]; then echo "int(r'${VIASH_PAR_LGBM_BOOST_ROUNDS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_early_stopping': $( if [ ! -z ${VIASH_PAR_LGBM_EARLY_STOPPING+x} ]; then echo "int(r'${VIASH_PAR_LGBM_EARLY_STOPPING//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'nn_epochs': $( if [ ! -z ${VIASH_PAR_NN_EPOCHS+x} ]; then echo "int(r'${VIASH_PAR_NN_EPOCHS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'n_tsvd_components': $( if [ ! -z ${VIASH_PAR_N_TSVD_COMPONENTS+x} ]; then echo "int(r'${VIASH_PAR_N_TSVD_COMPONENTS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_learning_rate': $( if [ ! -z ${VIASH_PAR_LGBM_LEARNING_RATE+x} ]; then echo "float(r'${VIASH_PAR_LGBM_LEARNING_RATE//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_max_bin': $( if [ ! -z ${VIASH_PAR_LGBM_MAX_BIN+x} ]; then echo "int(r'${VIASH_PAR_LGBM_MAX_BIN//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'lgbm_n_jobs': $( if [ ! -z ${VIASH_PAR_LGBM_N_JOBS+x} ]; then echo "int(r'${VIASH_PAR_LGBM_N_JOBS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi )
}
meta = {
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_predict_modality/methods/senkin_tmp_train",
    "tag" : "build_main"
  },
  "label" : [
    "highmem",
